...
prepreki = ()#((1, 4), (1, 6), (1, 8), (2, 3), (2, 9), (4, 2), (4, 7), (4, 8), (5, 5), (5, 7), (6, 1), (6, 2), (6, 4), (6, 7))


def DesnoX(atom, drugi):
    X, Y = atom
    while Y < 9 and (X, Y + 1) not in drugi and (X, Y + 1) not in prepreki:
        Y += 1
    return (X, Y)


def LevoX(atom, drugi):
    X, Y = atom
    while Y > 1 and (X, Y - 1) not in drugi and (X, Y - 1) not in prepreki:
        Y -= 1
    return (X, Y)


def GoreX(atom, drugi):
    X, Y = atom
    while X > 1 and (X - 1, Y) not in drugi and (X - 1, Y) not in prepreki:
        X -= 1
    return (X, Y)


def DoluX(atom, drugi):
    X, Y = atom
    while X < 7 and (X + 1, Y) not in drugi and (X + 1, Y) not in prepreki:
        X += 1
    return (X, Y)
...
class Molekula(Problem):
    ...
    def goal_test(self, state):
        ...
        return H1X == OX and OX == H2X and H1Y + 1 == OY and OY + 1 == H2Y
    ...
    def successor(self, state):
        ...
        # GoreH1
        goreh1 = GoreX(H1, (O, H2))
        if goreh1 != H1:
            successors["GoreH1"] = (goreh1, O, H2)
        # DoluH1
        doluh1 = DoluX(H1, (O, H2))
        if doluh1 != H1:
            successors["DoluH1"] = (doluh1, O, H2)
        # LevoH1
        levoh1 = LevoX(H1, (O, H2))
        if levoh1 != H1:
            successors["LevoH1"] = (levoh1, O, H2)
        # DesnoH1
        desnoh1 = DesnoX(H1, (O, H2))
        if desnoh1 != H1:
            successors["DesnoH1"] = (desnoh1, O, H2)

        # GoreO
        goreo = GoreX(O, (H1, H2))
        if goreo != O:
            successors["GoreO"] = (H1, goreo, H2)
        # DoluO
        doluo = DoluX(O, (H1, H2))
        if doluo != O:
            successors["DoluO"] = (H1, doluo, H2)
        # LevoO
        levoo = LevoX(O, (H1, H2))
        if levoo != O:
            successors["LevoO"] = (H1, levoo, H2)
        # DesnoO
        desnoo = DesnoX(O, (H1, H2))
        if desnoo != O:
            successors["DesnoO"] = (H1, desnoo, H2)

        # GoreH2
        goreh2 = GoreX(H2, (O, H1))
        if goreh2 != H2:
            successors["GoreH2"] = (H1, O, goreh2)
        # DoluH2
        doluh2 = DoluX(H2, (O, H1))
        if doluh2 != H2:
            successors["DoluH2"] = (H1, O, doluh2)
        # LevoH2
        levoh2 = LevoX(H2, (O, H1))
        if levoh2 != H2:
            successors["LevoH2"] = (H1, O, levoh2)
        # DesnoH2
        desnoh2 = DesnoX(H2, (O, H1))
        if desnoh2 != H2:
            successors["DesnoH2"] = (H1, O, desnoh2)

        return successors

    def h(self, node):
        H1, O, H2 = node.state
        return math.ceil(mhd(H1, O) / 8) + math.ceil(mhd(O, H2) / 8)
...
if __name__ == '__main__':
    ...
    instanca = Molekula(((h1_atom_row, h1_atom_column), (o_atom_row, o_atom_column), (h2_atom_row, h2_atom_column)))
