...
def buildtree(data, scoref = entropy):
    ...
    tree1 = build_tree(first_half, scoref)
    tree2 = build_tree(second_half, scoref)
    return (tree1, tree2)
...
def print_tree2(tree, level = 0, indent=''):
    # Дали е ова лист јазел?
    if tree.results:
        print(str(tree.results))
    else:
        ...
        # Се печатат True гранките, па False гранките
        print(indent + 'T-> ', end='')
        print_tree2(tree.tb, level + 1, indent + '  ')
        print(indent + 'F-> ', end='')
        print_tree2(tree.fb, level + 1, indent + '  ')
...
def classify(observation, tree):
    tree1, tree2 = tree
    dict1 = classify2(observation, tree1)
    dict2 = classify2(observation, tree2)

    klasa = None
    num = -1
    for (key, value) in dict1.items():
        if (value == num and key < klasa) or value > num:
            klasa = key
            num = value
