...
if __name__ == "__main__":
    ...
    validation_set = dataset[-5:]

    input_layer = len(dataset[0]) - 1
    output_layer = len(set([row[-1] for row in dataset]))

    network3 = initialize_network(input_layer, 3, output_layer)
    network5 = initialize_network(input_layer, 3, output_layer)
    network7 = initialize_network(input_layer, 3, output_layer)

    train_network(network3, training_set, 0.3, 20, output_layer, False)
    train_network(network5, training_set, 0.5, 20, output_layer, False)
    train_network(network7, training_set, 0.7, 20, output_layer, False)

    correct3 = correct5 = correct7 = 0

    for sample in validation_set:
        if predict(network3, sample) == sample[-1]:
            correct3 += 1
        if predict(network5, sample) == sample[-1]:
            correct5 += 1
        if predict(network7, sample) == sample[-1]:
            correct7 += 1

    if correct3 > correct5 and correct3 > correct5:
        print(predict(network3, testCase))
    elif correct5 > correct7:
        print(predict(network5, testCase))
    else:
        print(predict(network7, testCase))