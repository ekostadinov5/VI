...
class GraphProblem(Problem):
    ...
    def h(self, node):
        ...
        if locs:
            ...
            return hops(self.graph, node.state, self.goal) * 80


def hops(graph, current, goal):
    sub_problem = GraphProblem(current, goal, graph)
    return len(breadth_first_graph_search(sub_problem).solution())
...
instanca = GraphProblem(Pocetok, Kraj, graph)
#resenie = uniform_cost_search(instanca)
resenie = astar_search(instanca)

print(resenie.path_cost)