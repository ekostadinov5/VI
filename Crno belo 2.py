...
def click(state, x, y):
    ...
    newState = tuple([tuple(row) for row in newState])
    return newState
...
class CrnoBelo(Problem):
    ...
    def successor(self, state):
        ...
        for x in range(N):
            for y in range(N):
                successors["x: " + str(x) + ", y: " + str(y)] = click(state, x, y)

        return successors

    def h(self, node):
        return math.ceil(numWhite(node.state) / (2 * len(node.state) - 1))
