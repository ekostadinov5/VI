...
vrski = ((1, 2), (1, 5), (2, 6), (3, 4), (3, 7), (4, 8), (5, 6), (6, 7), (6, 10), (6, 11), (7, 8), \
         (7, 11), (9, 10), (9, 13), (10, 11), (10, 14), (11, 12), (11, 15), (12, 16), (13, 14), (15, 16))

class IstrazhuvashVoGraf(Problem):
    def __init__(self, covece, Z1, Z2):
        super().__init__((covece, (Z1, Z2), vrski))
    ...
    def successor(self, state):
        ...
        # Levo
        if (covece - 1) % 4 > 0 and (left, covece) in vrski:
            newVrski = tuple([v for v in vrski if v != (left, covece)])
            newZvezdi = tuple([z for z in zvezdi if z != left])
            successors["Levo"] = (left, newZvezdi, newVrski)

        # Desno
        if (covece - 1) % 4 < 3 and (covece, right) in vrski:
            newVrski = tuple([v for v in vrski if v != (covece, right)])
            newZvezdi = tuple([z for z in zvezdi if z != right])
            successors["Desno"] = (right, newZvezdi, newVrski)

        # Gore
        if covece > 4 and (up, covece) in vrski:
            newVrski = tuple([v for v in vrski if v != (up, covece)])
            newZvezdi = tuple([z for z in zvezdi if z != up])
            successors["Gore"] = (up, newZvezdi, newVrski)

        # Dolu
        if covece < 13 and (covece, down) in vrski:
            newVrski = tuple([v for v in vrski if v != (covece, down)])
            newZvezdi = tuple([z for z in zvezdi if z != down])
            successors["Dolu"] = (down, newZvezdi, newVrski)

        # DoluDesno
        if covece == 6 and (covece, downRight) in vrski:
            newVrski = tuple([v for v in vrski if v != (covece, downRight)])
            newZvezdi = tuple([z for z in zvezdi if z != downRight])
            successors["DoluDesno"] = (downRight, newZvezdi, newVrski)

        # GoreLevo
        if covece == 11 and (upLeft, covece) in vrski:
            newVrski = tuple([v for v in vrski if v != (upLeft, covece)])
            newZvezdi = tuple([z for z in zvezdi if z != upLeft])
            successors["GoreLevo"] = (upLeft, newZvezdi, newVrski)

        return successors


    def h(self, node):
        covece, zvezdi, vrski = node.state
        if zvezdi == ():
            return 0
        elif len(zvezdi) == 1:
            return mhd(point(covece), point(zvezdi[0]))
        else:
            return min(mhd(point(covece), point(zvezdi[0])), mhd(point(covece), point(zvezdi[1]))) + mhd(point(zvezdi[0]), point(zvezdi[1]))
...
def point(pos):
    return (int((pos - 1) / 4), int ((pos - 1) % 4))
...
instanca = IstrazhuvashVoGraf(IgracPozicija, Z1Pozicija, Z2Pozicija)
