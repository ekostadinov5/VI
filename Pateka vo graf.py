...
class GraphProblem(Problem):
    ...
    def __init__(self, initial, goal, special, graph):
    ...
    def path_cost(self, c, state1, action, state2):
        cost = c + (self.graph.get(state1, state2) or math.inf)
        if(state2 == self.special):
            return cost - 9
        else:
            return cost
...
instanca = GraphProblem(Pocetok, Kraj, Stanica, graph)
