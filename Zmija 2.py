...
class Zmija(Problem):
    def __init__(self, zeleni):
        super().__init__((((0, 0), (1, 0), (2, 0)), "Down", tuple(zeleni)))
        self.crveni = () #crveni
    ...
    def successor(self, state):
        ...
        zmija, pravec, zeleni = state
        X, Y = zmija[-1]

        #ProdolzhiPravo
        if pravec == "Up" and (X - 1, Y) not in self.crveni and (X - 1, Y) not in zmija and X > 0:
            if (X - 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X - 1, Y))
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Up", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X - 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X - 1, Y)]
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Up", tuple(newZeleni))

        if pravec == "Down" and (X + 1, Y) not in self.crveni and (X + 1, Y) not in zmija and X < 9:
            if (X + 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X + 1, Y))
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Down", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X + 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X + 1, Y)]
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Down", tuple(newZeleni))

        if pravec == "Left" and (X, Y - 1) not in self.crveni and (X, Y - 1) not in zmija and Y > 0:
            if (X, Y - 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y - 1))
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Left", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y - 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y - 1)]
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Left", tuple(newZeleni))

        if pravec == "Right" and (X, Y + 1) not in self.crveni and (X, Y + 1) not in zmija and Y < 9:
            if (X, Y + 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y + 1))
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Right", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y + 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y + 1)]
                successors["ProdolzhiPravo"] = (tuple(newZmija), "Right", tuple(newZeleni))

        #SvrtiDesno
        if pravec == "Left" and (X - 1, Y) not in self.crveni and (X - 1, Y) not in zmija and X > 0:
            if (X - 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X - 1, Y))
                successors["SvrtiDesno"] = (tuple(newZmija), "Up", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X - 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X - 1, Y)]
                successors["SvrtiDesno"] = (tuple(newZmija), "Up", tuple(newZeleni))

        if pravec == "Right" and (X + 1, Y) not in self.crveni and (X + 1, Y) not in zmija and X < 9:
            if (X + 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X + 1, Y))
                successors["SvrtiDesno"] = (tuple(newZmija), "Down", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X + 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X + 1, Y)]
                successors["SvrtiDesno"] = (tuple(newZmija), "Down", tuple(newZeleni))

        if pravec == "Down" and (X, Y - 1) not in self.crveni and (X, Y - 1) not in zmija and Y > 0:
            if (X, Y - 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y - 1))
                successors["SvrtiDesno"] = (tuple(newZmija), "Left", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y - 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y - 1)]
                successors["SvrtiDesno"] = (tuple(newZmija), "Left", tuple(newZeleni))

        if pravec == "Up" and (X, Y + 1) not in self.crveni and (X, Y + 1) not in zmija and Y < 9:
            if (X, Y + 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y + 1))
                successors["SvrtiDesno"] = (tuple(newZmija), "Right", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y + 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y + 1)]
                successors["SvrtiDesno"] = (tuple(newZmija), "Right", tuple(newZeleni))

        #SvrtiLevo
        if pravec == "Right" and (X - 1, Y) not in self.crveni and (X - 1, Y) not in zmija and X > 0:
            if (X - 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X - 1, Y))
                successors["SvrtiLevo"] = (tuple(newZmija), "Up", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X - 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X - 1, Y)]
                successors["SvrtiLevo"] = (tuple(newZmija), "Up", tuple(newZeleni))

        if pravec == "Left" and (X + 1, Y) not in self.crveni and (X + 1, Y) not in zmija and X < 9:
            if (X + 1, Y) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X + 1, Y))
                successors["SvrtiLevo"] = (tuple(newZmija), "Down", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X + 1, Y))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X + 1, Y)]
                successors["SvrtiLevo"] = (tuple(newZmija), "Down", tuple(newZeleni))

        if pravec == "Up" and (X, Y - 1) not in self.crveni and (X, Y - 1) not in zmija and Y > 0:
            if (X, Y - 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y - 1))
                successors["SvrtiLevo"] = (tuple(newZmija), "Left", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y - 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y - 1)]
                successors["SvrtiLevo"] = (tuple(newZmija), "Left", tuple(newZeleni))

        if pravec == "Down" and (X, Y + 1) not in self.crveni and (X, Y + 1) not in zmija and Y < 9:
            if (X, Y + 1) not in zeleni:
                newZmija = list(zmija[1:])
                newZmija.append((X, Y + 1))
                successors["SvrtiLevo"] = (tuple(newZmija), "Right", zeleni)
            else:
                newZmija = list(zmija)
                newZmija.append((X, Y + 1))
                newZeleni = [jabolka for jabolka in zeleni if jabolka != (X, Y + 1)]
                successors["SvrtiLevo"] = (tuple(newZmija), "Right", tuple(newZeleni))

        return successors

    def h(self, node):
        zmija, pravec, zeleni = node.state
        num_jabolki = len(zeleni)
        if num_jabolki == 0:
            return 0
        else:
            return min([mhd(zmija[-1], jabolko) for jabolko in zeleni]) + num_jabolki
...
if __name__ == '__main__':
    ...
    instanca = Zmija(zeleni_jabolki)
    resenie = astar_search(instanca)
