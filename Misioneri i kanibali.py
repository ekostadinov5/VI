...
def valid_state(state):
    ML, KL, B, MD, KD = state
    return (ML <= 0 or ML >= KL) and (MD <= 0 or MD >= KD) and (ML >= 0 and KL >= 0 and MD >= 0 and KD >= 0)


class MisionerIKanibali(Problem):
    def __init__(self, N, K):
        super().__init__((N, N, "L", 0, 0), (0, 0, "D", N, N))
        self.K = K
    ...
    def successor(self, state):
        ...
        for k in range(1, K + 1):
            if B == "L":
                newState = (ML, KL - k, "D", MD, KD + k)
                if (valid_state(newState)):
                    successors[str(0) + "M_" + str(k) + "K"] = newState
            else:
                newState = (ML, KL + k, "L", MD, KD - k)
                if (valid_state(newState)):
                    successors[str(0) + "M_" + str(k) + "K"] = newState
        for m in range(1, K + 1):
            for k in range(0, K - m + 1):
                if B == "L":
                    newState = (ML - m, KL - k, "D", MD + m, KD + k)
                    if(valid_state(newState)):
                        successors[str(m) + "M_" + str(k) + "K"] = newState
                else:
                    newState = (ML + m, KL + k, "L", MD - m, KD - k)
                    if (valid_state(newState)):
                        successors[str(m) + "M_" + str(k) + "K"] = newState

        return successors

    def h(self, node): #NOT ADMISSABLE!
        approx = math.ceil((node.state[0] + node.state[1]))#/ self.K)
        if node.state[2] == "L":
            return approx
        else:
            return approx + 1
...
instanca = MisionerIKanibali(N, K)
